...
export const TeamSection: React.FC = () => {
  ...
  return (
    <section id="team" className="py-28 md:py-36 relative bg-white border-t border-slate-200">
      {/* Glow background */}
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[700px] h-[350px] bg-[#1d4ed8]/5 rounded-full blur-[140px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10 relative z-10">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {founders.map((founder, index) => (
            <motion.div
              ...
              className="bg-white rounded-[3rem] p-8 sm:p-10 border border-slate-200 hover:border-[#1d4ed8]/30 transition-all duration-300 flex flex-col justify-between group shadow-xl shadow-slate-900/5"
            >
              <div>
                ...
                <div className="flex items-start gap-6 mb-6">
                  <div className="relative shrink-0">
                    <img
                      src={founder.image}
                      alt={founder.name}
                      className="w-24 h-24 sm:w-28 sm:h-28 rounded-3xl object-cover border-2 border-[#1d4ed8]/30 shadow-lg shadow-[#1d4ed8]/10 group-hover:scale-105 transition-transform duration-300"
                    />
                    <div className="absolute -bottom-2 -right-2 p-1.5 rounded-xl bg-slate-50 border border-[#1d4ed8]/30 text-[#1d4ed8]">
                      <Brain className="w-4 h-4" />
                    </div>
                  </div>

                  <div>
                    <span className="inline-block text-[10px] font-black uppercase tracking-wider text-[#1d4ed8] bg-[#1d4ed8]/10 px-2.5 py-1 rounded-md mb-2">
                      {founder.role}
                    </span>
                    <h3 className="text-xl sm:text-2xl font-manrope font-black text-slate-900 leading-tight mb-1">
                      {founder.name}
                    </h3>
                ...
                <div className="flex flex-wrap gap-2 mb-8">
                  {founder.credentials.map((cred, i) => (
                    <span
                      key={i}
                      className="text-[11px] font-semibold px-3 py-1 rounded-xl bg-slate-50 border border-slate-200 text-slate-600"
                    >
                      {cred}
                    </span>
                  ))}
              ...
              <div className="pt-6 border-t border-slate-200 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
                <a
                  ...
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-50 hover:bg-[#1d4ed8]/15 border border-slate-200 hover:border-[#1d4ed8]/30 text-xs font-bold text-slate-900 transition-all"
                >
        ...
        <div className="mt-14 max-w-2xl mx-auto p-6 rounded-2xl bg-white/60 border border-slate-200 text-center text-xs text-slate-500 flex items-center justify-center gap-3">
