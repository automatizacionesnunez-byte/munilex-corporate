...
export const ContactSection: React.FC<ContactSectionProps> = ({ targetType, initialPlan, initialStudents }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    phone: '',
    organization: '',
    role: isAcademy ? 'Director / Propietario de Academia' : 'Equipo Directivo / Jefatura de FP',
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-start">
          ...
          <div className="lg:col-span-5 space-y-8">
            <div className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border text-xs font-bold uppercase tracking-widest ${
              ...
              <span>{isAcademy ? 'Contacto Directo B2B Academias' : 'Contacto Institucional FP'}</span>
            </div>
            ...
            <p className="text-base text-white/60 leading-relaxed">
              {isAcademy
                ? 'Agenda una sesión personalizada de 20 minutos donde te mostraremos la plataforma en vivo con tus colores corporativos y calcularemos tu propuesta económica exacta.'
            ...
            <div className="p-4 rounded-2xl bg-white/[0.03] border border-white/10 flex items-center gap-3 text-xs text-white/70">
              <Clock className={`w-5 h-5 shrink-0 ${isAcademy ? 'text-[#d4af37]' : 'text-cyan-400'}`} />
              <div>
            ...
            <div className="space-y-4 pt-2">
              <a
                ...
                className="flex items-center gap-4 p-4 rounded-2xl bg-[#13141B] border border-white/5 hover:border-emerald-500/40 transition-all group"
              >
                <div className="w-12 h-12 rounded-xl bg-[#0B0C10] flex items-center justify-center text-emerald-400 group-hover:scale-110 transition-transform">
                  <MessageCircle className="w-5 h-5" />
                </div>
                <div>
                  <div className="text-xs text-white/40 font-medium">Dirección Comercial & Oposiciones</div>
              ...
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-[#13141B] border border-white/5">
                <div className={`w-12 h-12 rounded-xl bg-[#0B0C10] flex items-center justify-center ${
            ...
            <div className="p-6 rounded-3xl bg-[#13141B]/60 border border-white/5 space-y-3 text-xs text-white/60">
          ...
          <div className="lg:col-span-7">
            <div className="p-8 sm:p-12 rounded-[3rem] bg-[#13141B] border border-white/10 shadow-2xl shadow-black/80">
              {submitted ? (
                <motion.div
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-4 py-3.5 rounded-xl bg-[#0B0C10] border border-white/10 text-white placeholder-white/20 text-sm focus:border-[#d4af37] focus:outline-none transition-colors"
                      />
                    </div>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-xs font-bold uppercase tracking-wider text-white/70 mb-2">
                        {isAcademy ? 'Nombre de tu Academia *' : 'Nombre del Instituto / Centro FP *'}
                      </label>
                      <input
                        type="text"
                        required
                        placeholder={isAcademy ? 'Academia Oposiciones Élite' : 'IES / Centro de FP San Viator'}
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-4 py-3.5 rounded-xl bg-[#0B0C10] border border-white/10 text-white text-sm focus:border-[#d4af37] focus:outline-none transition-colors"
                      >
                    ...
                    {isAcademy ? (
                      <div>
                        ...
                        <select
                          ...
                          className="w-full px-4 py-3.5 rounded-xl bg-[#0B0C10] border border-white/10 text-white text-sm focus:border-[#d4af37] focus:outline-none transition-colors"
                        >
                          <option value="Plan Iniciación">Plan Iniciación (2 Oposiciones)</option>
                  ...
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wider text-white/70 mb-2">
                      {isAcademy
                        ? '¿Qué oposiciones o necesidades específicas tienes? (Opcional)'
                        : '¿Qué módulos o necesidades de simulación queréis reforzar? (Opcional)'}
                    </label>
                    <textarea
                      rows={3}
                      placeholder={isAcademy
                        ? 'Ej. Preparamos AGE y Policía Nacional. Nos interesa disponer de corrección automática de casos prácticos...'
                      ...
                      className="w-full px-4 py-3.5 rounded-xl bg-[#0B0C10] border border-white/10 text-white placeholder-white/20 text-sm focus:border-[#d4af37] focus:outline-none transition-colors resize-none"
                    />
                  </div>
                  ...
                  <button
                    ...
                    className={`w-full py-4 rounded-xl font-manrope font-black text-xs uppercase tracking-widest shadow-xl hover:scale-[1.01] active:scale-[0.99] transition-all cursor-pointer flex items-center justify-center gap-2 ${
                      isAcademy
                        ? 'bg-gradient-to-r from-[#d4af37] via-[#e6c65b] to-[#c79e27] text-black shadow-[#d4af37]/25'
